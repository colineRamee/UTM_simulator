import utm_simulator.environment as environment
import utm_simulator.layer_environment as layer_environment
import utm_simulator.centralized_manager as centralized_manager
...
class Simulation:
    def __init__(self, length, n_intruder, minimum_separation, max_speed,
                 ...
                 structure=None, demand=None, static_obstacles=None, n_priority_agents=0,
                 multiple_planning_agents=False, n_layers=1,
        ...
        if algorithm_type == 'SIPP':
            ...
            self.centralized_manager = centralized_manager.VOCentralizedManager(minimum_separation, size_area=length)
        elif algorithm_type == 'Decoupled':
            self.centralized_manager = centralized_manager.CentralizedManager(minimum_separation, size_area=length)
        else:
        ...
        self.n_priority_agents = n_priority_agents
        ...
        if n_layers == 1:
            self.env = environment.Environment(length, minimum_separation, max_speed, self.sensing_radius, self.tolerance, utm_on,
                                               ...
                                               structure=structure, demand=demand, static_obstacles=static_obstacles, priority_agents=n_priority_agents,
            ...
            self.env = layer_environment.LayerEnvironment(length, minimum_separation, max_speed, self.sensing_radius, self.tolerance, utm_on,
                                                          desired_number_of_agents=n_intruder, centralized_manager=self.centralized_manager,
                                                          multiple_planning_agents=multiple_planning_agents, structure=structure, demand=demand,
                                                          static_obstacles=static_obstacles, priority_agents=n_priority_agents, simulation_type=simulation_type,
                                                          algo_type=algorithm_type, log_type=log_type, log_density=log_density,
                                                          n_valid_agents_for_simulation_end=n_valid_agents_for_simulation_end, logger=self.my_logger,
                                                          n_layers=n_layers)
    ...
    def _run(self):
        ...
        if not self.multiple_planning_agents:
            ...
            if self.algorithm_type != 'empty' and self.n_layers == 1:
                if self.simulation_type == 'reactive':
        ...
        while self.time < self.time_end and not finished:
            ...
            if self.algorithm_type == 'empty':
                finished = False
            if self.update_queue is not None:
                agents_display = self.env.get_all_agents()
        ...
        filename = 'logs/' + self.simulation_name
        if self.save_file:
    ...
    def log_inputs(self):
        ...
        if self.static_obstacles is not None:
            input_dic['static_obstacles'] = self.static_obstacles
        if self.n_priority_agents != 0:
            input_dic['n_priority_agents'] = self.n_priority_agents
        return input_dic
