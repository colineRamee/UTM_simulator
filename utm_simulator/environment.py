...
class Environment:
    def __init__(self, size_map, min_distance, agent_speed, sensing_radius, tolerance, utm_on, desired_number_of_agents=1, centralized_manager=None,
                 ...
                 algo_type='A_star_8', log_type='short', log_density=False, n_valid_agents_for_simulation_end=None, logger=MyLogger()):
    ...
    def run(self, time, sim_dt):
        ...
        if self.time - self.debug_time >= 600:
            self.my_logger.log('sim time is ' + str(time) + ', density is ' + str(len(self.smart_agents) + len(self.dumb_agents)) + 'and there are ' + str(len(self.waiting_agent)) + ' waiting to take-off')
        ...
        if self.ramping_up:
            if len(self.smart_agents) + len(self.waiting_agent) >= self.desired_number_of_agents:
        ...
        if self.ramping_up_priority:
            if len(self.dumb_agents) >= self.n_priority_agents:
                self.ramping_up_priority = False
                self.my_logger.log('environment reached the desired number of priority agents at ' + str(self.time + self.dt))
        ...
        for a in self.smart_agents + self.dumb_agents:
            ...
            if a.status == 'stuck':
                ...
                if self.log_type != 'short' or a.agent_logic != 'dumb':
                    self.agent_record_list.append(a.log_agent())
                if self.t0 is not None and a.agent_logic != 'dumb' and a.desired_start_time >= self.t0:
    ...
    def add_priority_agent(self, start_time=0.0, density=0):
        self.total_number_of_agents += 1
        start, end = self.get_random_start_and_end_outside()
        if self.utm_on:
            a = agent.Agent(self, self.min_distance, self.speed, start=start, end=end, start_time=start_time, agent_logic='dumb', id=self.total_number_of_agents,
                            centralized_manager=self.centralized_manager)
            flightplan = a.preflight(self.default_planning_time_step, density=density)
            self.centralized_manager.add_flight_plan(flightplan, a.id, a, start_time, priority=True)
        else:
            a = agent.Agent(self, self.min_distance, self.speed, start=start, end=end, start_time=start_time, agent_logic='dumb', id=self.total_number_of_agents)
    ...
    def add_random_agent(self, random_type='edge', start_time=0.0, density=0):
        ...
        if self.utm_on:
            flightplan = a.preflight(self.default_planning_time_step, density=density)
            self.centralized_manager.add_flight_plan(flightplan, a.id, a, start_time)
    ...
    def add_intruder(self, start=None, end=None, delay=0, density=0):
        ...
        if self.utm_on:
            flightplan = a.preflight(self.default_planning_time_step, density=density)
            self.centralized_manager.add_flight_plan(flightplan, a.id, a, delay)
        self.dumb_agents.append(a)
        return a
    ...
    def add_strategic_agent(self, start=None, end=None, start_time=0.0, algo_type='A_star', density=0, flight_leg='inital'):
        ...
        if self.centralized_manager is None:
            ...
            self.total_number_of_agents += 1
            a = agent.Agent(self, self.min_distance, self.speed, start=start, end=end, start_time=start_time, agent_logic='strategic', algo_type=algo_type,
