...
class ORCA:
    def __init__(self):
        ...
        self.max_distance_to_obstacles = 5000
        self.time_horizon_obstacle = 250
        self.inv_time_horizon = 1 / self.time_horizon
        self.inv_time_horizon_obs = 1 / self.time_horizon_obstacle
    ...
    def compute_new_velocity(self, agent, dt):
        ...
        k = 0.1  # Slight increase to take care of floating point errors for static obstacles
        max_inflation_distance = 5
        ...
        static_obstacles = agent.get_nearest_obstacles(self.k_obstacles, self.max_distance_to_obstacles)
        for obstacle in static_obstacles:
            # In the C++ RVO2 implementation static obstacles are polygons defined by their vertices
            # Here the obstacles are all circles of varying radius
            # The differences with a neighbor are: radius, 0 velocity, do not share avoidance effort, constraints should not be relaxed
            # Construct ORCA lines
            # Project on the half plane
            rel_position = obstacle.position - agent.position
            d = np.linalg.norm(rel_position)
            # Opposite for some reason
            # Relative velocity computed using current velocity
            rel_velocity = agent.velocity
            # Is there a collision?
            inflated_radius = obstacle.radius + min(k * obstacle.radius, max_inflation_distance)
            if d > inflated_radius:
                ...
                if dot_product1 < 0 and dot_product1 ** 2 > inflated_radius ** 2 * np.dot(w, w):
                    ...
                    if np.linalg.det([rel_position, w]) > 0:
                        ...
                        # Unit vector
                        direction = np.array([rel_position[0] * leg - rel_position[1] * inflated_radius,
                                              rel_position[0] * inflated_radius + rel_position[1] * leg]) / d ** 2
                    else:
                        # On right leg
                        direction = - np.array([rel_position[0] * leg + rel_position[1] * inflated_radius,
                                                - rel_position[0] * inflated_radius + rel_position[1] * leg]) / d ** 2
                ...
                # Already colliding with obstacle, pick the velocity that will get us out within the time step
                w = rel_velocity - rel_position / dt
                w_norm = np.linalg.norm(w)
                unit_w = w / w_norm
                u = (obstacle.radius / dt - w_norm) * unit_w
                direction = np.array([unit_w[1], -unit_w[0]])
            # Contrary to the intruder the agent is responsible for all the avoidance effort with a static obstacle
            line = Line(agent.velocity + u, direction)
            orca_lines.append(line)
        n_obstacles = len(orca_lines)
        ...
        for neighbor in neighbors:
            ...
            if d > R:
                ...
                unit_w = w / w_norm
                # dot_product1<0 is a necessary but not sufficient condition for the projection having to be done on the cutoff circle
                # The second condition compares the angle between w and - rel_position (lambda) to the angle between rel_position and the radius tangent to the line (alpha)
                # Project on the circle if lambda < alpha => cos^2 (lambda) > cos^2(alpha) because alpha and lambda are between 0 and pi/2
                # cos^2 lambda = dot_product1**2/(|w|^2*|rel_position|^2)
                # cos^2 alpha = R^2 / |rel_position|^2
                ...
                w_norm = np.linalg.norm(w)
                if w_norm == 0:
                    print('Colliding with intruder')
                    print('Relative velocity ' + str(rel_velocity))
                    ...
                    direction = np.array([unit_w[1], -unit_w[0]])
            if u is not None:
                # Shared collision avoidance effort only if the neighbor collaborates
                if neighbor.agent_logic != 'dumb':
        ...
        if line_fail < len(orca_lines):
            ...
            new_vel = self.linear_program3(orca_lines, line_fail, agent.maxSpeed, pref_vel, num_obstacles=n_obstacles)
        ...
        for i in range(0, line_no):
            ...
            if abs(denom) <= self.epsilon:
                ...
                if numer < 0:
                    ...
                    # I think it should not happen given how the solution is computed
                    return False, opt_v
                else:
                    continue
            t = numer / denom
    ...
    def linear_program3(self, lines, line_fail, max_speed, opt_v, num_obstacles=0):
        ...
        for i in range(line_fail, len(lines)):
            if np.linalg.det([lines[i].direction, lines[i].point - result]) > distance:
                ...
                if num_obstacles != 0:
                    projected_lines = lines[0:num_obstacles]
                else:
                    projected_lines = []
